...
bool Logger::initialize() {
    ...
    // Populate IMU queue
    if (imu_success) {
        add_to_queue(DataStream::IMU);
    }

    if (left_mono_success) {
        add_to_queue(DataStream::LEFT_MONO);
    }

    if (right_mono_success) {
        add_to_queue(DataStream::RIGHT_MONO);
    }

    if (rgb_mono_success) {
        add_to_queue(DataStream::RGB);
    }
...
TimePoint Logger::read_imu_packet(const dai::IMUPacket &packet) {
    ...
    // Update preview
    preview_.update_imu(static_cast<float>(time_since_start(timestamp)));
...
void Logger::start_logging() {
    ...
    TimePoint last_sensor_timestamp = TimePoint::max();
    while (!quit_) {
        const auto opt_last_sensor_timestamp = log_queues();
        if (opt_last_sensor_timestamp) {
            last_sensor_timestamp = *opt_last_sensor_timestamp;
            if (!start_ts_) {
    ...
    LOG(ERROR) << "Finished logging after "
               << time_since_start(last_sensor_timestamp) << " s";
    LOG(ERROR) << serializer_.output_file_info();
}

void Logger::replay(std::string_view input_file) {
    // Check if input file is ok to read, if not, open it.
    if (!serializer_.ok_to_read()) {
...
std::optional<DataStream> Logger::sequential_read(std::string_view input_file,
                                                  IMUPacket &imu_packet,
                                                  CameraPacket &cam_packet) {
    // Check if input file is open already, if not, open it
    if (!serializer_.ok_to_read()) {
        if (!serializer_.prepare_input_stream(input_file)) {
            LOG(ERROR) << "Error reading from file " << input_file;
    ...
    return serializer_.read_input_stream(imu_packet, cam_packet);
}
...
bool Logger::configure_and_add_rgb_camera() {
    ...
    camera_node->setFps(CAM_FPS);
    camera_node->setPreviewSize(CAM_COLS, CAM_ROWS);
...
void Logger::add_to_queue(const DataStream &type) {
...
float Logger::time_since_start(const TimePoint &time) const {
